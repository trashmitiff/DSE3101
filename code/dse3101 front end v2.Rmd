  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = "accessibility",
        ...
        fluidRow(
          column(width = 6,
            box(title = "🗺️ Transport Accessibility Map", width = NULL, height = "850px",
                leafletOutput("accessibility_map", height = "575px"),
                selectInput("map_transport_mode", "Select Transport Mode:", 
                choices = c("All" = "acc_score_scaled",
                            "Bus" = "bus_acc_score_scaled",
                            "MRT" = "mrt_acc_score_scaled")),
                conditionalPanel(
                  condition = "input.map_transport_mode == 'acc_score_scaled'",
                  sliderInput("time_of_day", "Select Hour of Day:",
                              min = 5, max = 24, value = 12, step = 1)
                )
            )
          ),
          column(width = 6, 
            box(title = "📊 Transport Accessibility Data", width = NULL, height = "850px",
              tabsetPanel(
                tabPanel("Table",
                  textInput("subzone_search", "🔍 Search for an Area:", ""),
                ...
                tabPanel("Chart", 
                  selectInput("chart_transport_mode", "Select Transport Mode:",
                    choices = c("All" = "avg_acc_score", 
                                "Bus" = "avg_bus_acc_score", 
                                "MRT" = "avg_mrt_acc_score")),
...
server <- function(input, output, session) {
  ...
  output$accessibility_map <- renderLeaflet({
    ...
    if (input$map_transport_mode == "acc_score_scaled") {
      
      hour_data <- hourly_data %>%
        filter(hour == ifelse(input$time_of_day == 24, 0, input$time_of_day)) %>%
      ...
      addTiles() %>%
      addPolygons(
        fillColor = ~colorQuantile("Reds", accessibility_score)(accessibility_score), 
        fillOpacity = 0.7,
        color = "white",
        weight = 1,
        popup = ~paste0("<b>", subzone_name, "</b><br>Accessibility Score: ", accessibility_score)
      ) %>%
      addLegend(position = "bottomright",
        colors = c("#B71C1C", "#D32F2F", "#EF5350", "#FFCDD2", "#FFEBEE"),
        labels = c("Very High", "High", "Medium", "Low", "Very Low"),
    ...
    } else if (input$map_transport_mode == "bus_acc_score_scaled") {
      accessibility_map_data <- accessibility_subzone_data %>%
        mutate(accessibility_score = get(input$map_transport_mode))
      pal <- colorBin(
        palette = "Greens",
        domain = accessibility_map_data$accessibility_score,
        bins = c(0, 0.05, 0.1, 0.15, 0.2, 0.4, 0.6, 0.8, 1)
      )
      ...
      pal <- colorBin(
        palette = "Blues",
        domain = accessibility_map_data$accessibility_score,
        bins = c(0, 0.02, 0.04, 0.06, 0.08, 0.1, 0.2, 0.4, 1)
      )
  ...
  output$accessibility_chart <- renderPlotly({
    selected_score <- input$chart_transport_mode
